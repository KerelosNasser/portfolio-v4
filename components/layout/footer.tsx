import React from "react";
import { Container } from "./container";
import { profileData } from "@/lib/data/profile";
import { ArrowUpRight, Mail, MessageCircle } from "lucide-react";

function GithubIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      {...props}
    >
      <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
      <path d="M9 18c-4.51 2-5-2-7-2" />
    </svg>
  );
}

function LinkedinIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      {...props}
    >
      <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
      <rect width="4" height="12" x="2" y="9" />
      <circle cx="4" cy="4" r="2" />
    </svg>
  );
}

export function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="border-t border-white/[0.08] bg-[#07090e] py-12 text-sm">
      <Container>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-4 lg:grid-cols-5">
          {/* Col 1: Bio / Identity */}
          <div className="md:col-span-2 lg:col-span-2 space-y-3">
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 items-center justify-center rounded bg-white/[0.08] border border-white/[0.12] text-xs font-mono font-bold text-white">
                KN
              </span>
              <span className="font-semibold text-white tracking-tight">
                {profileData.name}
              </span>
            </div>
            <p className="text-xs leading-relaxed text-slate-400 max-w-sm">
              Full Stack Developer building modern web applications, SaaS MVPs, dashboards,
              and internal tools from concept to deployment.
            </p>
            <div className="flex items-center gap-2 text-xs text-slate-500 font-mono pt-1">
              <span>Alexandria, Egypt</span>
              <span>•</span>
              <span className="text-emerald-400">Open to remote & freelance</span>
            </div>
          </div>

          {/* Col 2: Navigation */}
          <div className="space-y-3">
            <p className="font-mono text-xs uppercase tracking-wider text-slate-400">
              Navigation
            </p>
            <ul className="space-y-2 text-xs">
              <li>
                <a href="#work" className="text-slate-300 hover:text-white transition-colors">
                  Selected Work
                </a>
              </li>
              <li>
                <a href="#capabilities" className="text-slate-300 hover:text-white transition-colors">
                  Capabilities
                </a>
              </li>
              <li>
                <a href="#process" className="text-slate-300 hover:text-white transition-colors">
                  Process
                </a>
              </li>
              <li>
                <a href="#about" className="text-slate-300 hover:text-white transition-colors">
                  About
                </a>
              </li>
              <li>
                <a href="#contact" className="text-slate-300 hover:text-white transition-colors">
                  Contact
                </a>
              </li>
            </ul>
          </div>

          {/* Col 3: Direct Connect */}
          <div className="space-y-3 md:col-span-1 lg:col-span-2">
            <p className="font-mono text-xs uppercase tracking-wider text-slate-400">
              Direct Contact
            </p>
            <ul className="space-y-2 text-xs">
              <li>
                <a
                  href={profileData.contact.whatsapp.chatUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-slate-300 hover:text-emerald-400 transition-colors"
                >
                  <MessageCircle className="h-3.5 w-3.5" />
                  <span>WhatsApp ({profileData.contact.whatsapp.displayNumber})</span>
                  <ArrowUpRight className="h-3 w-3 opacity-60" />
                </a>
              </li>
              <li>
                <a
                  href={`mailto:${profileData.contact.email}`}
                  className="inline-flex items-center gap-1.5 text-slate-300 hover:text-white transition-colors"
                >
                  <Mail className="h-3.5 w-3.5" />
                  <span>{profileData.contact.email}</span>
                  <ArrowUpRight className="h-3 w-3 opacity-60" />
                </a>
              </li>
              <li>
                <a
                  href={profileData.contact.github}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-slate-300 hover:text-white transition-colors"
                >
                  <GithubIcon className="h-3.5 w-3.5" />
                  <span>GitHub</span>
                  <ArrowUpRight className="h-3 w-3 opacity-60" />
                </a>
              </li>
              <li>
                <a
                  href={profileData.contact.linkedin}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-slate-300 hover:text-white transition-colors"
                >
                  <LinkedinIcon className="h-3.5 w-3.5" />
                  <span>LinkedIn</span>
                  <ArrowUpRight className="h-3 w-3 opacity-60" />
                </a>
              </li>
            </ul>
          </div>
        </div>

        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-white/[0.06] pt-6 text-xs text-slate-500 sm:flex-row">
          <p>© {currentYear} Kerolos Nasser. Built with Next.js & Tailwind CSS.</p>
          <p className="font-mono">Clean code • Real projects • Verified deliverables</p>
        </div>
      </Container>
    </footer>
  );
}
